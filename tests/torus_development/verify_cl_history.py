...
final_values = {}
pattern = r'^\s*1\s+(\d+)\s+\d+\s+\d+\s+NK\s+----\s+[\d\.]+\s+[\d\.]+\s+([-\d\.E\+]+)\s+([-\d\.E\+]+)\s+([-\d\.E\+]+)'
...
for line in lines:
    ...
    if match:
        ...
        final_values[instance] = {'cl': cl, 'cd': cd, 'residual': residual}
...
# Print results
print("="*80)
print("TORUS TIME SPECTRAL - CL HISTORY VERIFICATION")
print("="*80)
...
for alpha_key in sorted(alpha_groups.keys()):
    ...
    if len(instances) > 1:
        ...
        if cl_diff < 1e-10:  # Very tight tolerance
...
if all_match:
    ...
    print("   Degenerate torus case VERIFIED")
else:
