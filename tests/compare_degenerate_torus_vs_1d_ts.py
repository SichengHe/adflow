...
def run_degenerate_torus():
    ...
    for i1 in range(n1):
        for i2 in range(n2):
            ...
            idx = i2 * n1 + i1
            print(f"  Instance {idx+1} ({i1},{i2}): α = {alpha_deg:7.3f}°")
    ...
    # Extract CL values
    cl_torus = extract_cl_from_output(output_text, ntot)
    ...
    for i in range(ntot):
        ...
        print(f"  Instance {i+1} ({i1},{i2}): α={alpha_deg:7.3f}°  CL={cl_torus[i]:.10f}")

    return cl_torus, alpha_grid, theta1


def run_1d_time_spectral():
    """Run regular 1D time spectral with 3 instances"""

    print("\n" + "="*70)
    print("RUNNING 1D TIME SPECTRAL (3 instances)")
    print("="*70)
    ...
    for i in range(n_instances):
        print(f"  Instance {i+1}: θ={theta[i]:.3f}, α={alpha_deg[i]:7.3f}°")
    ...
    # Extract CL values
    cl_1d = extract_cl_from_output(output_text, n_instances)

    print("\n1D Time Spectral CL values extracted:")
    for i in range(n_instances):
        print(f"  Instance {i+1}: θ={theta[i]:.3f}, α={alpha_deg[i]:7.3f}°  CL={cl_1d[i]:.10f}")

    return cl_1d, alpha_deg
...
def main():
    ...
    cl_torus, alpha_grid, theta1 = run_degenerate_torus()

    # Run 1D time spectral
    cl_1d, alpha_1d = run_1d_time_spectral()
    ...
    print("\n1D Time Spectral CL values (3 instances):")
    for i in range(len(cl_1d)):
        print(f"  Instance {i+1}: α={alpha_1d[i]:7.3f}°  CL={cl_1d[i]:.10f}")

    # Find matching alpha values
    print("\n" + "="*70)
    print("MATCHING ALPHA VALUES")
    print("="*70)

    print("\nLooking for torus instances that match 1D TS alpha values...")
    print()

    for j, alpha_1d_val in enumerate(alpha_1d):
        print(f"1D TS Instance {j+1}: α={alpha_1d_val:7.3f}°, CL={cl_1d[j]:.10f}")
        ...
        for i in range(len(cl_torus)):
            ...
            if abs(alpha_torus - alpha_1d_val) < 0.01:  # Within 0.01 degrees
                matches.append((i+1, i1, i2, alpha_torus, cl_torus[i]))

        if matches:
            for match in matches:
                inst, i1, i2, alpha_t, cl_t = match
                diff = abs(cl_t - cl_1d[j])
                status = "✅ MATCH" if diff < 1e-6 else "❌ MISMATCH"
                print(f"  → Torus Instance {inst} ({i1},{i2}): α={alpha_t:7.3f}°, CL={cl_t:.10f}, Δ={diff:.2e} {status}")
        print()
