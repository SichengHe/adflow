...
!  differentiation of bcdataisothermalwall in reverse (adjoint) mode (with options i4 dr8 r8 noisize):
...
!   gradient     of useful results: tref href pref *(*bcdata.ptinlet)
!                *(*bcdata.ttinlet) *(*bcdata.htinlet) bcvararray
!   with respect to varying inputs: tref href pref *(*bcdata.ptinlet)
!                *(*bcdata.ttinlet) *(*bcdata.htinlet) bcvararray
!   plus diff mem management of: bcdata:in *bcdata.ptinlet:in *bcdata.ttinlet:in
!                *bcdata.htinlet:in
!=================================================================
...
&               + pref*tempd
          ...
          do j=jend,jbeg,-1
            do i=iend,ibeg,-1
              rhot = mult*bcvararray(i, j, 3) + trans
              temp2 = rhot*tref/pref
              tempd1 = bcdata(boco)%ttinlet(i, j)*rgasdim*bcdatad(boco)%&
&               ptinlet(i, j)/pref
              bcdatad(boco)%ttinlet(i, j) = bcdatad(boco)%ttinlet(i, j) &
&               + rgasdim*temp2*bcdatad(boco)%ptinlet(i, j)
              rhotd = tref*tempd1
              trefd = trefd + rhot*tempd1
              prefd = prefd - temp2*tempd1
...
!  differentiation of bcdatasupersonicinflow in reverse (adjoint) mode (with options i4 dr8 r8 noisize):
...
!                *(*bcdata.turbinlet) *(*bcdata.rho) *(*bcdata.velx)
!                *(*bcdata.vely) *(*bcdata.velz) *(*bcdata.ps)
...
&             3)*vloc(3))*tempd/uref
            bcdatad(boco)%velz(i, j) = 0.0_8
            tempd0 = bcdatad(boco)%vely(i, j)/uref
            vlocd(1) = vlocd(1) + axis(2)*tempd0
            vlocd(2) = vlocd(2) + radvec1(2)*tempd0
            vlocd(3) = vlocd(3) + radvec2(2)*tempd0
            urefd = urefd - (axis(2)*vloc(1)+radvec1(2)*vloc(2)+radvec2(&
&             2)*vloc(3))*tempd0/uref
            bcdatad(boco)%vely(i, j) = 0.0_8
            tempd1 = bcdatad(boco)%velx(i, j)/uref
            vlocd(1) = vlocd(1) + axis(1)*tempd1
            vlocd(2) = vlocd(2) + radvec1(1)*tempd1
            vlocd(3) = vlocd(3) + radvec2(1)*tempd1
            urefd = urefd - (axis(1)*vloc(1)+radvec1(1)*vloc(2)+radvec2(&
&             1)*vloc(3))*tempd1/uref
            bcdatad(boco)%velx(i, j) = 0.0_8
            call popreal8(vloc(3))
            vradd = var*r2*vlocd(3)
            vthetad = vthetad + var*r1*vlocd(3)
            vlocd(3) = 0.0_8
            call popreal8(vloc(2))
            vradd = vradd + var*r1*vlocd(2)
            vthetad = vthetad - var*r2*vlocd(2)
            vlocd(2) = 0.0_8
...
&             *bcdatad(boco)%velz(i, j)/uref**2
            bcdatad(boco)%velz(i, j) = 0.0_8
            bcvararrayd(i, j, 4) = bcvararrayd(i, j, 4) + multvel(2)*&
&             bcdatad(boco)%vely(i, j)/uref
            urefd = urefd - (transvel(2)+multvel(2)*bcvararray(i, j, 4))&
&             *bcdatad(boco)%vely(i, j)/uref**2
            bcdatad(boco)%vely(i, j) = 0.0_8
            bcvararrayd(i, j, 3) = bcvararrayd(i, j, 3) + multvel(1)*&
&             bcdatad(boco)%velx(i, j)/uref
            urefd = urefd - (transvel(1)+multvel(1)*bcvararray(i, j, 3))&
&             *bcdatad(boco)%velx(i, j)/uref**2
            bcdatad(boco)%velx(i, j) = 0.0_8
          end do
        end do
      end if
...
!  differentiation of setbcvarturb in reverse (adjoint) mode (with options i4 dr8 r8 noisize):
...
&               bcvarnames(mm), mult, trans)
      ...
      do nn=nt2,nt1,-1
        ...
        if (branch .eq. 0) then
          ...
          do j=jend,jbeg,-1
            do i=iend,ibeg,-1
              tempd0 = turbinletd(i, j, nn)/ref(nn)
              bcvararrayd(i, j, mm) = bcvararrayd(i, j, mm) + mult*&
&               tempd0
...
&               tempd0/ref(nn)
      ...
      if (branch .lt. 2) then
        ...
        tmpd3 = refd(itu4)
        refd(itu4) = 0.0_8
        refd(itu1) = refd(itu1) + tmpd3/nuref
        nurefd = -(ref(itu1)*tmpd3/nuref**2)
        prefd = prefd + refd(itu1)/rhoref
        rhorefd = rhorefd - pref*refd(itu1)/rhoref**2
      end if
