...
!  differentiation of timespectralmatrices in forward (tangent) mode (with options i4 dr8 r8):
!   variations   of useful results: *dvector *dscalar
!   with respect to varying inputs: *dvector *dscalar
!   rw status of diff variables: *dvector:in-out *dscalar:in-out
!   plus diff mem management of: sections:in dvector:in-out dscalar:in-out
  subroutine timespectralmatrices_d()
!
...
!added to allow second call in mdupdateroutines
      if (allocated(dscalar)) then
        deallocate(dscalard)
        deallocate(dscalar)
      end if
      if (allocated(dvector)) then
        deallocate(dvectord)
        deallocate(dvector)
      end if
      ...
      mm = 3*nn
      kk = nn - 1
      allocate(dscalard(nsections, nn, nn), stat=ierr)
      allocate(dscalar(nsections, nn, nn), stat=ierr)
      allocate(dvectord(nsections, mm, mm), stat=ierr)
      ...
      if (ierr .ne. 0) call terminate('timespectralmatrices', &
...
! actual matrices.
      diagmatcoefspectrald(1:nsections, 1:3, 1:3) = 0.0_8
      matrixcoefspectrald(1:nsections, 1:kk, 1:3, 1:3) = 0.0_8
      coefspectrald(1:nsections, 1:kk) = 0.0_8
      call timespectralcoef_d(coefspectral, coefspectrald, &
&                       matrixcoefspectral, matrixcoefspectrald, &
&                       diagmatcoefspectral, diagmatcoefspectrald)
!
...
! the matrices of the time derivatives.
      deallocate(coefspectral, stat=ierr)
      deallocate(matrixcoefspectral, stat=ierr)
      deallocate(diagmatcoefspectral, stat=ierr)
      if (ierr .ne. 0) call terminate('timespectralmatrices', &
&                         'deallocation failure for the help variables.'&
&                              )
    end if
  end subroutine timespectralmatrices_d
!  differentiation of timespectralcoef in forward (tangent) mode (with options i4 dr8 r8):
!   variations   of useful results: diagmatcoefspectral coefspectral
!                matrixcoefspectral
!   with respect to varying inputs: timeref *sections.timeperiod
!                diagmatcoefspectral coefspectral matrixcoefspectral
!   rw status of diff variables: timeref:in *sections.timeperiod:in
!                diagmatcoefspectral:in-out coefspectral:in-out
!                matrixcoefspectral:in-out
!   plus diff mem management of: sections:in
  subroutine timespectralcoef_d(coefspectral, coefspectrald, &
&   matrixcoefspectral, matrixcoefspectrald, diagmatcoefspectral, &
&   diagmatcoefspectrald)
!
...
!
    real(kind=realtype), dimension(nsections, ntimeintervalsspectral - 1&
&   ), intent(out) :: coefspectral
    real(kind=realtype), dimension(nsections, ntimeintervalsspectral-1),&
&   intent(out) :: coefspectrald
    real(kind=realtype), dimension(nsections, ntimeintervalsspectral - 1&
&   , 3, 3), intent(out) :: matrixcoefspectral
    real(kind=realtype), dimension(nsections, ntimeintervalsspectral-1, &
&   3, 3), intent(out) :: matrixcoefspectrald
    real(kind=realtype), dimension(nsections, 3, 3), intent(out) :: &
&   diagmatcoefspectral
    real(kind=realtype), dimension(nsections, 3, 3), intent(out) :: &
&   diagmatcoefspectrald
!
...
! and the 2*pi/timeperiod
      ...
      coefd = (pi*timerefd*sections(mm)%timeperiod-pi*timeref*sectionsd(&
&       mm)%timeperiod)/sections(mm)%timeperiod**2
...
! latter cos/sin or 1/tan.
        coefspectrald(mm, nn) = coefd/sin(angle)
        coefspectral(mm, nn) = coef/sin(angle)
        if (mod(ntimeintervalsspectral, 2_inttype) .eq. 0) then
          coefspectrald(mm, nn) = cos(angle)*coefspectrald(mm, nn)
...
&       mm)%timeperiod)/sections(mm)%timeperiod**2
      ...
      do j=1,3
        do i=1,3
          diagmatcoefspectrald(mm, i, j) = coefd*diagmatcoefspectral(mm&
&           , i, j) + coef*diagmatcoefspectrald(mm, i, j)
...
&           , j)
      ...
      do nn=1,ntimeintervalsspectral-1
        do j=1,3
          do i=1,3
            matrixcoefspectrald(mm, nn, i, j) = coefd*matrixcoefspectral&
&             (mm, nn, i, j) + coef*matrixcoefspectrald(mm, nn, i, j)
...
&             mm, nn, i, j)
  ...
  end subroutine timespectralcoef_d
